...
export default function StockEntryItems() {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Delete ${selectedRows.size} selected roll entr${selectedRows.size === 1 ? 'y' : 'ies'}?`)) return

    setIsSubmitting(true)
    try {
      // Delete all selected rows
      const deletePromises = Array.from(selectedRows).map(id => deleteRollsStock(id))
      ...
      console.error("Error deleting rolls stock:", err)
      setError("Failed to delete selected rolls stock")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleBulkPrint = async () => {
    ...
    if (!window.confirm(`Print ${selectedRows.size} selected roll entr${selectedRows.size === 1 ? 'y' : 'ies'}?`)) return

    setIsPrinting(true)
    try {
      // Get selected rows
      const selectedRowsData = rollsStock.filter(row => row.id && selectedRows.has(row.id))
      ...
      setError("Failed to print selected rolls stock")
      setPrintStatus("idle")
    } finally {
      setIsPrinting(false)
    }
  ...
  const handleToggleRowSelection = (rowId: number | undefined) => {
    if (!rowId) return // Don't allow selecting unsaved rows
                  ...
                  {row.isEditing ? (
                    <>
                      <TableCell className="py-1 px-2">
                        {/* Empty cell for checkbox column in editing mode */}
                      </TableCell>
